...
void PrintSensingMatrix(const sensing_data_t *sdata) {
    if (!sdata || !sdata->sensinginfo) return;
...
void process_new_sm_data(const char *data, size_t len) {
    ...
    int total_len = 0;
    memcpy(&total_len, data + offset, sizeof(int));
    offset += sizeof(int);

    msg.len = (uint16_t) total_len;
    // fprintf(stdout, "[SM NEW] msg.len %i len %i", total_len, len);
    if (offset + total_len > len) {
        fprintf(stderr, "[SM NEW]: Packet length mismatch\n");
        return;
    }

    if (total_len < sizeof(float) * 6 + sizeof(int)) {
        fprintf(stderr, "[SM NEW]: Header too small\n");
        return;
    }
    ...
    fprintf(latencyfile, "%li\t %i\n", latest_new_data.tstamp, len); 
    // PrintSensingMatrix(latest_new_data.data);
    // PrintRawData(data, len);
    // PrintRawData(latest_new_data.data, latest_new_data.len);
}
...
void *tcp_server_func(void *sockPtr)
{
  ...
  char recvbuffer[500000];
  pid_t tid = gettid();


  char sendBuffer[1024];        // ejemplo de buffer de envío
  snprintf(sendBuffer, sizeof(sendBuffer), "ACK\n");
  size_t sendLen = strlen(sendBuffer);
  int sendPending = 0;          // indica si hay datos por enviar
  ...
  while (1)
  {
    ...
    int ready_to_send = flaghasctrldata;
    ...
    int maxfd = sock + 1;
    int activity = select(maxfd, &readfds, &writefds, NULL, NULL);
    if (activity < 0) {
        perror("select");
        break;
        }
    // ==== RECEPCIÓN ====
      if (FD_ISSET(sock, &readfds)) {
          ssize_t len = recv(sock, recvbuffer, BUFFER_SIZE, 0);
          if (len <= 0) {
              fprintf(stderr, "[TCP SERVER (%d)]: Recv <= 0 Bytes: %s\n", tid, strerror(errno));
              break;
          }

          // Leer cabecera de tamaño
          int totalLen = 0;
          memcpy(&totalLen, recvbuffer, sizeof(totalLen));
          totalLen += sizeof(totalLen);

          uint64_t accLen = len;
          fprintf(stdout, "[TCP SERVER (%d)] First received %lu/%d Bytes\n", tid, accLen, totalLen);

          // Recibir lo que falte
          while (accLen < totalLen) {
              ssize_t l = recv(sock, recvbuffer + accLen, totalLen - accLen, 0);
              if (l <= 0) {
                  fprintf(stderr, "[TCP SERVER (%d)]: Recv <= 0 Bytes: %s\n", tid, strerror(errno));
                  break;
              }
              accLen += l;
              printf("[TCP SERVER (%d)]: %ld Bytes received (%lu/%d)\n", tid, l, accLen, totalLen);
          }

          printf("[TCP SERVER (%d)]: Total %lu/%d Bytes received\n", tid, accLen, totalLen);
          process_new_sm_data(recvbuffer, accLen);

          
      }

      // ==== ENVÍO ====
      if (ready_to_send && FD_ISSET(sock, &writefds)) {
          pthread_mutex_lock(&ctr_req_data_mutex);

          ssize_t sent = send(sock, send_buf, send_buf_len, 0);
          if (sent <= 0) {
              perror("[TCP SERVER] Send failed");
              break;
          }
          printf("[TCP SERVER (%d)]: Sent %ld bytes\n", tid, sent);
          flaghasctrldata = false; // datos enviados
          pthread_mutex_unlock(&ctr_req_data_mutex);
      }
  }
  ...
  //   }
  //   printf("[TCP SERVER (%d)]: Total %i/%i Bytes recieved\n", tid, accLen, totalLen);
...
{
  // Signal handler
  const char* filename = "e2sensectr.txt" ;
  latencyfile = fopen(filename, "w");
  if (!latencyfile) {
