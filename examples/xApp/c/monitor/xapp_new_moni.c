...
void sm_cb_slice(sm_ag_if_rd_t const* rd, global_e2_node_id_t const* e2_node)
{
  ...
  printf("SLICE ind_msg latency = %ld from E2-node type %d ID %d\n",
         now - rd->ind.new_sm.msg.tstamp, e2_node->type, e2_node->nb_id.nb_id);
}
...
void sm_cb_new(sm_ag_if_rd_t const* rd, global_e2_node_id_t const* e2_node)
{
  ...
  int64_t now = time_now_us();
  printf("NEW! heatmap update - ind_msg latency = %ld ms from E2-node type %d ID %d\n",
         (now - rd->ind.new_sm.msg.tstamp)/1000, e2_node->type, e2_node->nb_id.nb_id);
 
  // printf("Len: %i/%i\n", rd->ind.new_sm.msg.len, sizeof(pixel_t));
  // for (uint16_t i = 0; i < rd->ind.new_sm.msg.len / sizeof(pixel_t); ++i) {
  //     printf("[xAPP DATA] Pixel[%d] = (%f, %f, %f)\n", i,
  //             rd->ind.new_sm.msg.data[i].r,  rd->ind.new_sm.msg.data[i].theta,  rd->ind.new_sm.msg.data[i].strength);
  // }
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < nodes.len; i++) {
    ...
    for (int32_t j = 0; j < args.sub_cust_sm_len; j++) {
      ...
      } else if (!strcasecmp(args.sub_cust_sm[j].name, "new")) {
        printf("xApp subscribes RAN Func ID %d in E2 node idx %d, nb_id %d\n", SM_NEW_ID, i, n->id.nb_id.nb_id);
        new_handle[i] = report_sm_xapp_api(&nodes.n[i].id, SM_NEW_ID, (void*)args.sub_cust_sm[j].time, sm_cb_new);
  ...
  for(int i = 0; i < nodes.len; ++i) {
    ...
    for (int32_t j = 0; j < args.sub_cust_sm_len; j++) {
      if (!strcasecmp(args.sub_cust_sm[j].name, "mac")) {
        ...
        printf("calling rm_report_sm_xapp_api\n");
