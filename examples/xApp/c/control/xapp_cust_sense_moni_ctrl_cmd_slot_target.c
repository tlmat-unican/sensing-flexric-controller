...
static void sm_cb_new(sm_ag_if_rd_t const* rd, global_e2_node_id_t const* e2_node) {
    ...
    if (((double) (now_us - last_tc_change_us)/1000.0) > min_tc_interval_ms) {
        ...
        printf("[PID] lat_slot: %.2f | lat_count: %i | late_ref: %.1f| avg: %.2f | pid_out: %.3f \n", lat_slot, lat_count, latency_ref_ms,avg_lat_slot, pid_output);
...
int main(int argc, char *argv[]) {
    ...
    double lat_values[4] = {80.0, 60.0, 40.0, 80.0};
    // while(1){
        int n = 0;
        while(n<4){
            latency_ref_ms = (double) lat_values[n];
            sleep(20);
            n++;
        }
    // }
