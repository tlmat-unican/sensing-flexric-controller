...
def main():
    ...
    try:
        # data fetching loop
        while True:
            timeperiod = active_params['period']
            ...
            if ctrl_params["flagtoread"]:
                ...
                headerdata = struct.pack(headerFmt, ctrl_params["theta_min"], cols, ctrl_params['theta_max'], r[0,0], rows, r[0,-1], precision)
            else:
                precision = 1
                # timeperiod = 1
                ...
                headerFmt = '<6fi'
                rows, cols = z.shape
                headerdata = struct.pack(headerFmt, theta[0,0]/np.pi*180, cols, theta[-1,0]/np.pi*180, r[0,0], rows, r[0,-1], precision)
            ...
            print(f"Enviado ({packet_size} bytes) "
                  f"period={timeperiod}s, precision={precision}, "
