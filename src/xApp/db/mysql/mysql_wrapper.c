...
// static
// void create_new_table(MYSQL* conn){


//   assert(conn != NULL);

//   // Eliminar la tabla si ya existe
//   if (mysql_query(conn, "DROP TABLE IF EXISTS PIXEL"))
//     mysql_finish_with_error(conn);

//   // Crear la tabla PIXEL
//   char* sql_pixel = "CREATE TABLE PIXEL("
//                     "r FLOAT NOT NULL,"                 
...
{
    assert(conn != NULL);
    // printf("Create Sensing data Table\n");
    // Eliminar la tabla si ya existe
    if (mysql_query(conn, "DROP TABLE IF EXISTS SENSING_DATA")) {
        mysql_finish_with_error(conn);
    }

    // Crear la tabla SENSING_DATA
    const char* sql_sensing =  
        "CREATE TABLE SENSING_DATA ("
        "    radio FLOAT NOT NULL,"
        "    angulo FLOAT NOT NULL,"
        "    intensidad FLOAT NOT NULL,"
        "    PRIMARY KEY (radio, angulo)"
        ");";

    if (mysql_query(conn, sql_sensing)) {
        mysql_finish_with_error(conn);
    }
}
...
static int to_mysql_string_new(const pixel_t* px, char* out, size_t out_len)
{
    assert(px != NULL);
    assert(out != NULL);

    return snprintf(out, out_len, "(%.6f, %.6f, %.6f)", px->r, px->theta, px->strength);
}
...
/*INSERT INTO PIXEL (r, theta, strength) VALUES (_, _, _)
ON DUPLICATE KEY UPDATE strength = VALUES(strength);*/
// char pixels_buffer[2048] = "INSERT INTO PIXEL (r, theta, strength) VALUES";
// char end_buffer[2000] = " ON DUPLICATE KEY UPDATE strength = VALUES(strength);";
// char pixel_temp[9000000] = "";
// static
// void write_new_stats(MYSQL* conn, global_e2_node_id_t const* id, new_ind_data_t const* ind)
// {
    
//     assert(conn != NULL);
//     assert(ind != NULL);

//     size_t pixel_size = 12;
//     const new_ind_msg_t* ind_msg_mew = &ind->msg;
//     int pixels_max =  ind_msg_mew->len / pixel_size;
    
//     for (uint16_t i = 0; i < pixels_max; ++i) {
//         char tuple_buffer[257] = "";
//         const char* pixel_data = ind_msg_mew->data + (i * pixel_size);
//         //  printf("[xAPP-MySQL DATA] Pixel[%d] = (%f, %f, %f)\n", i,
//         //       ind_msg_mew->data[i].r, ind_msg_mew->data[i].theta,  ind_msg_mew->data[i].strength);
//         // Convertimos datos binarios del pixel a string SQL
        
//         to_mysql_string_new(&ind_msg_mew->data[i], tuple_buffer, sizeof(tuple_buffer));

//         // Si es el primer pixel, iniciamos el query
//         if (i == 0) {
//             strcpy(pixel_temp, pixels_buffer);
//             strcat(pixel_temp, tuple_buffer);
//         } else {
//             strcat(pixel_temp, ",");
//             strcat(pixel_temp, tuple_buffer);
//         }
...
//         // Cuando llegamos al límite, ejecutamos
//         if (i == pixels_max-1) {
//             strcat(pixel_temp, end_buffer);
//             //  printf("%s\n", pixel_temp);

//             if (mysql_query(conn, pixel_temp)) {
//                 mysql_finish_with_error(conn);
//             }
//             // Reset
//             strcpy(pixel_temp,"");
...
static int to_mysql_string_sensing(const sensing_data_t* s, char* out, size_t out_len, size_t sensinginfo_len)
{
    assert(s != NULL);
    assert(out != NULL);

   return snprintf(out, out_len,
        "(%.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %d, '%.*s')",
        s->theta_init,
        s->theta_step,
        s->theta_end,
        s->r_init,
...
char pixel_temp[10000000] = ""; // buffer grande para la query

static
void write_sensing_stats(MYSQL* conn, global_e2_node_id_t const* id, new_ind_data_t const* ind)
{
   assert(conn != NULL);
    assert(ind != NULL);

    const new_ind_msg_t* msg = &ind->msg;
    if (msg->len < 28) {
        printf("[MySQL] >> Sensing SM - No data or too short\n");
        return;
    }

    const uint8_t* buf = (const uint8_t*)msg->data;
    size_t sensinginfo_len = msg->len - 28;

    // Reservar estructura + espacio para la matriz
    sensing_data_t* data = malloc(sizeof(sensing_data_t) + sensinginfo_len);
    if (!data) {
    ...
    // Copiar metadatos
    memcpy(&data->theta_init, buf + 0,  sizeof(float));
    memcpy(&data->theta_step, buf + 4,  sizeof(float));
    memcpy(&data->theta_end,  buf + 8,  sizeof(float));
    memcpy(&data->r_init,     buf + 12, sizeof(float));
    memcpy(&data->r_step,     buf + 16, sizeof(float));
    memcpy(&data->r_end,      buf + 20, sizeof(float));
    memcpy(&data->precision,  buf + 24, sizeof(ElementType));

    // Copiar matriz en el flexible array
    memcpy(data->sensinginfo, buf + 28, sensinginfo_len);

    // Ahora puedes usar data->sensinginfo como un array de bytes
    size_t n_theta = (size_t)data->theta_step;
    size_t n_r     = (size_t)data->r_step;
   
    // Buffer para construir INSERT
    size_t buffer_size = 100 * 1024 * 1024; // 10 MB
    char* query_buffer = malloc(buffer_size);
    if (!query_buffer) {
        fprintf(stderr, "malloc failed\n");
        free(data);
        return;
    ...
    strcpy(query_buffer, "INSERT INTO SENSING_DATA (radio, angulo, intensidad) VALUES ");

    size_t offset = strlen(query_buffer);
    int first = 1;

     // Recorremos la matriz según el tipo de precision
    if (data->precision == TYPE_INT) {
        int32_t* arr = (int32_t*)data->sensinginfo;
        for (size_t i = 0; i < n_r; i++) {
            float radio = data->r_init + i * (data->r_end - data->r_init) / (n_r - 1);
            for (size_t j = 0; j < n_theta; j++) {
                float angulo = data->theta_init + j * (data->theta_end - data->theta_init) / (n_theta - 1);
                if (!first) strcat(query_buffer, ","); else first = 0;
                char temp[128];
                snprintf(temp, sizeof(temp), "(%f,%f,%d)", radio, angulo, arr[i * n_theta + j]);
                strcat(query_buffer, temp);
            }
        }
    } else if (data->precision == TYPE_FLOAT) {
        float* arr = (float*)data->sensinginfo;
        for (size_t i = 0; i < n_r; i++) {
            float radio = data->r_init + i * (data->r_end - data->r_init) / (n_r - 1);
            for (size_t j = 0; j < n_theta; j++) {
                float angulo = data->theta_init + j * (data->theta_end - data->theta_init) / (n_theta - 1);
                if (!first) strcat(query_buffer, ","); else first = 0;
                char temp[128];
                snprintf(temp, sizeof(temp), "(%f,%f,%f)", radio, angulo, arr[i * n_theta + j]);
                strcat(query_buffer, temp);
            }
        }
    } else if (data->precision == TYPE_DOUBLE) {
        double* arr = (double*)data->sensinginfo;
        for (size_t i = 0; i < n_r; i++) {
            double radio = data->r_init + i * (data->r_end - data->r_init) / (n_r - 1);
            for (size_t j = 0; j < n_theta; j++) {
                double angulo = data->theta_init + j * (data->theta_end - data->theta_init) / (n_theta - 1);
                if (!first) strcat(query_buffer, ","); else first = 0;
                char temp[128];
                snprintf(temp, sizeof(temp), "(%lf,%lf,%lf)", radio, angulo, arr[i * n_theta + j]);
                strcat(query_buffer, temp);
            }
        }
    }
    ...
    strcat(query_buffer, " ON DUPLICATE KEY UPDATE intensidad = VALUES(intensidad);");
...
void init_db_mysql(MYSQL** conn, db_params_t const* db_params)
{
  ...
  *conn = mysql_init(NULL);
  ...
  printf("[MySQL]: Create New SENSING Table Successful\n");
}
...
void write_db_mysql(MYSQL* conn, global_e2_node_id_t const* id, sm_ag_if_rd_t const* ag_rd)
{
  ...
  assert(rd->type == MAC_STATS_V0   || rd->type == RLC_STATS_V0
         ...
         || rd->type == KPM_STATS_V3_0 || rd->type == NEW_STATS_V0 || rd->type == RAN_CTRL_STATS_V1_03);
  ...
  if(rd->type == MAC_STATS_V0){
    ...
    write_sensing_stats(conn, id, &rd->new_sm);
