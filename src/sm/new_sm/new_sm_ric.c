...
sm_ag_if_rd_ind_t on_indication_new_sm_ric(sm_ric_t const* sm_ric, sm_ind_data_t const* data)
{
 ...
 sm_ag_if_rd_ind_t rd_if = {.type = NEW_STATS_V0};
  ...
  // Header
  rd_if.new_sm.msg = new_dec_ind_msg(&sm->enc, data->len_msg, data->ind_msg);

  // Message
  rd_if.new_sm.hdr = new_dec_ind_hdr(&sm->enc, data->len_hdr, data->ind_hdr);

  //  call_process_id
  assert(data->call_process_id == NULL && "not implemented"); 
  rd_if.new_sm.proc_id = NULL;
...
sm_ctrl_req_data_t ric_on_control_req_new_sm_ric(sm_ric_t const* sm_ric, void* ctrl)
{
      assert(sm_ric != NULL); 
    assert(ctrl != NULL); 
    printf("===== !!! ric_on_control_req_new_sm_ric\n");

    // new_ctrl_req_data_t const* req_in = (new_ctrl_req_data_t const*) ctrl;
    // new_ctrl_req_data_t req = *req_in; // copia local (ya no const)
    ...
    // byte_array_t ba_hdr = new_enc_ctrl_hdr(&sm->enc, &req.hdr);
    // ret_data.ctrl_hdr = ba_hdr.buf;
    // ret_data.len_hdr  = ba_hdr.len;

   
    // byte_array_t ba_msg = new_enc_ctrl_msg(&sm->enc, &req.msg);
    ...
    // // printf("Action before encoding: %u\n", req->msg.action);
  ...
  new_ctrl_req_data_t const* req = &wr->ctrl.new_ctrl;
...
sm_ag_if_ans_ctrl_t ric_on_control_out_new_sm_ric(sm_ric_t const* sm_ric,const sm_ctrl_out_data_t * out)
{
  ...
  sm_ag_if_ans_ctrl_t ag_if = {.type =  NEW_CTRL_IF_CTRL_ANS_V0};  
...
sm_ag_if_rd_e2setup_t ric_on_e2_setup_new_sm_ric(sm_ric_t const* sm_ric, sm_e2_setup_data_t const* setup)
{
  ...
  dst.new_sm.func_def.len = setup->len_rfd;
  if(dst.new_sm.func_def.len > 0){
    dst.new_sm.func_def.buf = calloc(dst.new_sm.func_def.len, sizeof(uint8_t));
    assert(dst.new_sm.func_def.buf != NULL && "Memory exhausted");
    memcpy(dst.new_sm.func_def.buf, setup->ran_fun_def, setup->len_rfd);
  }
...
void free_ind_data_new_sm_ric(void* msg)
{
  ...
  new_ind_data_t* ind = &rd_ind->new_sm;
